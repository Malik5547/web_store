...
class ShopTestCases(TestCase):
    ...
    def setUp(self) -> None:
        ...
        image = SimpleUploadedFile('notebook_image.jpg', content=b'asd', content_type='image/jpeg')
        self.notebook = Notebook.objects.create(
            category=self.category,
            title='testnotebook',
            slug='test-slug',
            image=SimpleUploadedFile(name='test_image.jpg', content=open('media/a.jpg', 'rb').read(), content_type='image/jpeg'),
        ...
        self.customer = Customer.objects.create(user=self.user, phone='234234234', address='asdasda')
    ...
    def test_add_to_cart(self):
        ...
        self.assertEqual(self.cart.total_price, Decimal(5000.00))
    ...
    def test_mock_homepage(self):
        ...
        with mock.patch('store.views.BaseView.get', return_value=mock_data) as mock_data:
            factory = RequestFactory()
            request = factory.get('')
            request.user = self.user
